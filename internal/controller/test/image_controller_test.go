...
var _ = Describe("Image Controller", func() {
	Context("When reconciling a resource", func() {
			...
			if err != nil && errors.IsNotFound(err) {
				resource := &automotivev1.Image{
					...
					Spec: automotivev1.ImageSpec{
						...
						Description: "Test image for controller testing",
					},
				}
		...
		It("should successfully reconcile the resource", func() {
			...
			controllerReconciler := &image.ImageReconciler{
				Client: k8sClient,
			...
			By("Checking the Image status is updated")
			Eventually(func() bool {
				err := k8sClient.Get(ctx, typeNamespacedName, img)
				if err != nil {
					return false
				}
				return img.Status.Phase != ""
			}).Should(BeTrue())
		})
	})
})
